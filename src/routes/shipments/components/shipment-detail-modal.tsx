...
export default function ShipmentDetailModal() {
  ...
  // Handle URL synchronization with protection against double execution
  useEffect(() => {
    const pathMatch = location.pathname.match(/\/shipments\/[^/]+\/(.+)$/);
    if (pathMatch && pathMatch[1]) {
      const shipmentIdFromUrl = pathMatch[1];

      // Only update if we have a different shipment ID
      if (shipmentIdFromUrl !== selectedShipmentId) {
    ...
    } else if (isDetailModalOpen && selectedShipmentId) {
      // If URL doesn't have shipment ID but modal is open, close it
  ...
  const handleClose = () => {
    setIsDetailModalOpen(false);
    setSelectedShipmentId(null);
    // Navigate back to the shipments page without the ID
  ...
  if (!isDetailModalOpen || !selectedShipmentId) {
    return null;
  }
